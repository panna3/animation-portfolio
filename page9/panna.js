...
class Panna{
  ...
  show(){
    ...
    let tailLength = 66;
    let tailRadius = min(windowWidth, windowHeight) /12;
    //tail
    for(let i = 5; i < tailLength;i++){
      let r = map(i,0,tailLength,0,tailRadius);
      let x = this.x-31 + cos(r+i)*r/7;
      let y = this.y+3 + sin(r+i)*r/7;
